/**
 * AuraChat Admin Dashboard JavaScript
 */
(function ($) {
    "use strict";
    ...
    $(document).ready(function () {
        // 1. Tab Navigation
        $(".aurachat-admin-tabs .nav-tab").on("click", function (e) {
            e.preventDefault();
            var targetTab = $(this).data("tab");

            $(".aurachat-admin-tabs .nav-tab").removeClass("nav-tab-active");
            $(this).addClass("nav-tab-active");

            $(".aurachat-tab-panel").removeClass("active");
        ...
        function toggleActiveProviderSettings() {
            var selectedProvider = $("#aurachat_ai_provider").val() || "gemini";
            $(".ai-provider-settings-card").hide();
            $("#settings-card-" + selectedProvider).show();
        }

        $("#aurachat_ai_provider").on("change", function () {
        ...
        $(document).on("click", ".aurachat-go-to-tab", function(e) {
            e.preventDefault();
            var targetTab = $(this).data("target-tab");
            $('.aurachat-admin-tabs .nav-tab[data-tab="' + targetTab + '"]').trigger("click");
        });

        // 3. Color Pickers
        if ($.fn.wpColorPicker) {
            $(".aurachat-color-picker").wpColorPicker({
                change: function () {
                    updateLivePreview();
                },
                clear: function () {
        ...
        $(".aurachat-radio-cards .radio-card input").on("change", function () {
            $(".aurachat-radio-cards .radio-card").removeClass("selected");
        ...
        $("#aurachat-add-response-btn").on("click", function (e) {
            e.preventDefault();
            var rowHtml =
                '<div class="rule-row">' +
                    '<div class="col-kw">' +
                        '<input type="text" name="aurachat_settings[responses_keyword][]" placeholder="e.g. discount, return" class="regular-text" required />' +
                    '</div>' +
                    '<div class="col-reply">' +
                        '<textarea name="aurachat_settings[responses_reply][]" rows="2" class="large-text" placeholder="Bot reply content..." required></textarea>' +
                    '</div>' +
                    '<div class="col-act">' +
                        '<button type="button" class="button button-link-delete aurachat-remove-row" title="Remove Rule">' +
                            '<span class="dashicons dashicons-trash"></span>' +
                        '</button>' +
                    '</div>' +
                '</div>';
            $("#aurachat-responses-container").append(rowHtml);
        });

        $(document).on("click", ".aurachat-remove-row", function (e) {
            e.preventDefault();
        ...
        function updateLivePreview() {
            ...
            var position = $('input[name="aurachat_settings[position]"]:checked').val() || "right";
